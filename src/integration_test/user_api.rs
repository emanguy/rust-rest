...
use actix_web::{
    http::StatusCode,
    test::{call_and_read_body_json, call_service, TestRequest},
    web::Bytes,
};

use crate::{dto::NewUser, routes::{self, InsertedUser}, db::TodoUser};
...
async fn can_retrieve_user() {
    ...
    let user_id =
        serde_json::from_slice::<InsertedUser>(res_body.expect("Could not read create user body").borrow())
            .expect("Could not parse create user response");

    let delete_user_req = TestRequest::get().uri(format!("/users").as_str()).to_request();
    ...
    let received_user: Vec<TodoUser> = serde_json::from_slice(res_body.expect("Could not extract user list body").borrow()).expect("Could not parse user list body");
